...
from ..models import Signal, SignalType, MinerTreasuryData, BlockData
from ..config import settings
from .base_processor import SignalProcessor, ProcessorConfig, ProcessingContext


class MinerProcessor(SignalProcessor):
    """
    ...
    def __init__(self, config: Optional[ProcessorConfig] = None):
        if config is None:
            config = ProcessorConfig(
                enabled=getattr(settings, 'miner_processor_enabled', True),
                confidence_threshold=getattr(settings, 'confidence_threshold', 0.7),
                time_window=getattr(settings, 'miner_time_window', '7d')
            )
        super().__init__(config)
        self.signal_type = "miner"
    ...
    async def process_block(
        ...
        """
        Process block and generate miner treasury signals
        ...
        Returns:
            List of miner signals above confidence threshold
        """
        signals = []
        
        # Extract miner data from context
        miner_data_list = context.historical_data.get('miner_data', [])
        if not miner_data_list:
            return signals
        
        # Get historical miner data for comparison
        historical_miner = context.historical_data.get('historical_miner', [])
        
        # Generate signal for each miner
        for miner_data in miner_data_list:
            signal = self.generate_miner_signal(miner_data, historical_miner)
    ...
    def generate_miner_signal(
        ...
        signal_data = {
            "pool_name": current_data.entity_name,
            "amount_btc": abs(current_data.daily_change_btc),
            "treasury_balance_change": current_data.daily_change_btc,
            "entity": entity_info,
